...
export class WhatsAppController {
    ...
    async init() {
        ...
        this.sock.ev.on("connection.update", (update: Partial<ConnectionState>) => {
            ...
            if (connection === "close") {
				...
				if (lastDisconnect?.error) {
					// type guard
					if ((lastDisconnect.error as Boom).output) {
        ...
        this.sock.ev.on("messages.upsert", async (m) => {
            ...
            const colors = ["red", "green", "yellow", "magenta", "cyan", "white", "blue"] as const;
            const color = colors[Math.floor(Math.random() * colors.length)];

            console.log(
                chalk.yellow.bold("Credit : Lenwy"),
                chalk.green.bold("[ WhatsApp ]"),
                chalk[color](pushname),
                chalk[color](":"),
    ...
    async sendMessage(jid: string, message: string) {
        ...
        const msg: AnyMessageContent = { text: message };
